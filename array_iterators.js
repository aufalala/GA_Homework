...
veryOldInventors = inventors.filter((inventor) => {
    return 1500<=inventor.year && 1599>=inventor.year;
});
...
sortedByBirthYear = inventors.sort((first, second) => {
...
totalYearsLived = inventors.reduce((acc, current) =>{
...
travelMethodCounts = travelMethods.reduce((acc, current) => {
